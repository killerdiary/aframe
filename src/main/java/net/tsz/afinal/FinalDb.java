...
public class FinalDb {
    ...
    public <T> T findByWhere(Class<T> clazz, String strWhere) {
        checkTableExist(clazz);
        List<T> list = findAllBySql(clazz,
     ...
     *            分组字段
     */
    public <T> List<T> findAllByGroup(Class<T> clazz, String strWhere,
            String gropBy) {
        checkTableExist(clazz);
        return findAllBySql(clazz,
                SqlBuilder.getSelectSQLByWhere(clazz, strWhere) + " GROUP BY "
                        + gropBy);
    }
    ...
    public <T> int getCount(Class<T> clazz, String strWhere) {
        ...
        sql.append(TableInfo.get(clazz).getTableName());
        if (strWhere != null) {
            sql.append(" WHERE ");
            sql.append(strWhere);
        }
     ...
     * @return
     */
    public <T> List<T> findAll(Class<T> clazz, String orderBy, int count) {
        return findAllBySql(clazz, SqlBuilder.getSelectSQL(clazz)
                + getOrderBy(orderBy) + " LIMIT " + count);
    }
     ...
     * @return
     */
    public <T> List<T> findAll(Class<T> clazz, String orderBy, int page,
            int count) {
        return findAllBySql(clazz, SqlBuilder.getSelectSQL(clazz)
                + getOrderBy(orderBy) + " LIMIT " + page * count + "," + count);
    }
    ...
    public <T> List<T> findAllByWhere(Class<T> clazz, String strWhere,
        ...
        return findAllBySql(clazz,
                SqlBuilder.getSelectSQLByWhere(clazz, strWhere)
                        + getOrderBy(orderBy) + " LIMIT " + page * count + ","
                        + count);
    }
    ...
    private String getOrderBy(String orderby) {
        ...
        if (orderby != null) {
            ...
            ob += " ORDER BY ";
            ob += orderby;
            if (orderby.indexOf("DESC") < 0 && orderby.indexOf("ASC") < 0) {
                ob += " DESC";
            }
        }
        return ob;
