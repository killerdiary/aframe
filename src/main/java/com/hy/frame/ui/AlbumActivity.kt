...
import com.hy.frame.adapter.AlbumAdapter
 ...
 */
class AlbumActivity : BaseActivity(), IAdapterLongListener<AlbumInfo>, PermissionUtil.IRequestPermissionListener {
    ...
    override fun initView() {
        ...
        rcyList?.layoutManager = GridLayoutManager(context, 3)
                ...
                when (msg.what) {
                    ...
                    WHAT_FETCH_COMPLETED -> {
                        ...
                        if (msg.obj != null && msg.obj is MutableList<*>) {
                            val temp = msg.obj as MutableList<AlbumInfo>
                            datas?.addAll(temp)
                        }
                        updateUI()
                    }
    ...
    override fun updateUI() {
        if (HyUtil.isEmpty(datas)) {
            showNoData("没有搜索到图片")
            return
        }
        showCView()
        if (adapter == null) {
            adapter = AlbumAdapter(context!!, datas, this, Glide.with(this))
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (resultCode == Activity.RESULT_OK) {
            val array = data?.getParcelableArrayListExtra<AlbumInfo>(AlbumPagerActivity.ARG_CHECK_DATA) ?: return
            checkDatas.clear()
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
        ...
        PermissionUtil.onRequestPermissionsResult(context!!, requestCode, permissions, grantResults, this)
    }
    ...
    companion object {
        private val WHAT_PERMISSION_GRANTED = 1
        private val WHAT_PERMISSION_DENIED = 2
        private val WHAT_FETCH_STARTED = 3
        private val WHAT_FETCH_COMPLETED = 4
