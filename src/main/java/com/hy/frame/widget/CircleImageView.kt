...
class CircleImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : android.support.v7.widget.AppCompatImageView(context, attrs, defStyle) {
    ...
    init {
        ...
        mFillColor = a.getColor(R.styleable.CircleImageView_civFillColor, DEFAULT_FILL_COLOR)
        a.recycle()
        init()
    }
    ...
    fun setBorderColorResource(@ColorRes borderColorRes: Int) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            borderColor = context.resources.getColor(borderColorRes, context.theme)
        } else {
    ...
    //            ColorDrawable cdraw = (ColorDrawable) background;
    //            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
    //                setBorderWidth(getPaddingTop());
    //                setBorderColor(cdraw.getColor());
    //            }
    //        }
    //    }
