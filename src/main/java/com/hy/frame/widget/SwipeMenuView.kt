...
class SwipeMenuView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ViewGroup(context, attrs, defStyleAttr) {
    ...
    private val LogUtils: android.util.Log? = null
     ...
     * @param ios
     */
    fun setIos(ios: Boolean): SwipeMenuView {
        isIos = ios
        return this
    }
     ...
     * @return
     */
    fun setLeftSwipe(leftSwipe: Boolean): SwipeMenuView {
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true
        val a = context.obtainStyledAttributes(attrs, R.styleable.SwipeMenuView, defStyleAttr, 0)
        ...
        if (isSwipeEnable) {
            ...
            when (ev.action) {
                MotionEvent.ACTION_DOWN -> {
                    ...
                    mFirstP.set(ev.rawX, ev.rawY)//2016 11 03 add,判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。
    ...
    private var isExpand: Boolean = false//代表当前是否是展开状态 2016 11 03 add
    ...
    override fun performLongClick(): Boolean {
        if (Math.abs(scrollX) > mScaleTouchSlop) {
            return false
        }
    ...
    companion object {
         ...
         * @return
         */
        var viewCache: SwipeMenuView? = null
            private set
